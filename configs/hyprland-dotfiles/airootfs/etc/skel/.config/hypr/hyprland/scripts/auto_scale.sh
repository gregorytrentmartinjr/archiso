...
#
# Results are written to monitors.conf so they persist across hyprctl reload.
...
apply_auto_scale() {
  ...
  # Wait for hyprctl to become available (monitors may not be ready immediately)
  local monitors=""
  local retries=0
  while [ $retries -lt 5 ]; do
    monitors=$(hyprctl monitors -j 2>/dev/null) && [ -n "$monitors" ] && [ "$monitors" != "[]" ] && break
    retries=$((retries + 1))
    sleep 1
  done

  if [ -z "$monitors" ] || [ "$monitors" = "[]" ]; then
    echo "auto_scale: no monitors detected after retries" >&2
    return 1
  fi

  # Collect monitor config lines
  local conf_lines=()
  local applied=false
  ...
  while read -r mon; do
    ...
    conf_lines+=("monitor=${name},preferred,auto,${scale}")
    hyprctl keyword monitor "$name,preferred,auto,$scale"
    applied=true
  done < <(echo "$monitors" | jq -c '.[]')

  # Write to monitors.conf so the scale persists across hyprctl reload
  if $applied; then
    {
      echo "# Auto-generated by auto_scale.sh — do not edit manually."
      echo "# To override, change scale in Settings > Display and apply."
      for line in "${conf_lines[@]}"; do
        echo "$line"
      done
    } > "$MONITORS_CONF"
    echo "auto_scale: wrote ${#conf_lines[@]} monitor(s) to $MONITORS_CONF"
  fi
}
